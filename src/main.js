...
let checkpointIndex = -1; // -1=start, 0..2=checkpoints completed
let itemsCollected = [];
...
function generateCharacterSVG(emotion) {
    ...
    ${emotion === 'happy' ? `
    <!-- Raised arms for happy -->
    <ellipse cx="42" cy="130" rx="12" ry="18" fill="${bodyColor}" transform="rotate(-40 42 130)"/>
    <ellipse cx="158" cy="130" rx="12" ry="18" fill="${bodyColor}" transform="rotate(40 158 130)"/>
    ` : ''}
    <!-- Legs -->
    ...
    if (!playerName) {
        ...
        if (saved) {
            ...
            for (let i = 0; i <= checkpointIndex; i++) {
                if (CHECKPOINTS[i]) {
                    itemsCollected.push(CHECKPOINTS[i].item);
                }
            }
        } else {
            currentPhase = 1;
...
function setCharacterImages() {
    ...
    if (charMap) charMap.src = encode(heroSVG);
}
...
function showPhase1() {
    ...
    // Play SOS sound after a brief delay
...
function handleShoesDone() {
    ...
    setTimeout(() => {
        transformStep.classList.remove('transform-enter');
    }, 800);
}
...
function updateMap() {
    ...
    const charMarker = document.getElementById('map-character');

    nodes.forEach((node, i) => {
        node.classList.remove('active', 'completed');
        const cp = node.dataset.cp;
        ...
        if (cp === 'start') {
            ...
            const cpIdx = parseInt(cp);
            if (cpIdx <= checkpointIndex) {
                node.classList.add('completed');
                node.querySelector('.node-icon').textContent = CHECKPOINTS[cpIdx].icon;
                node.querySelector('.node-label').textContent = CHECKPOINTS[cpIdx].name;
            } else if (cpIdx === checkpointIndex + 1) {
                node.classList.add('active');
    ...
    // Position character marker
    if (charMarker) {
        const activeIdx = checkpointIndex + 1; // 0-indexed node position (start=0, cp1=1...)
        const totalNodes = nodes.length;
        const progress = Math.min((activeIdx + 1) / totalNodes, 1);
        charMarker.style.left = `${progress * 100}%`;
    }

    // Update checkpoint button text
    if (checkpointIndex >= CHECKPOINTS.length - 1) {
        btnCheckpoint.innerHTML = '🏠 おうちとうちゃく！';
        btnCheckpoint.classList.add('btn-goal');
    } else {
        btnCheckpoint.innerHTML = '📍 チェックポイント！';
...
function updateMapMessage() {
    if (checkpointIndex < 0) {
        mapMessage.innerHTML = DIALOGUES.missionStart;
    } else if (checkpointIndex < CHECKPOINTS.length) {
        const nextCp = CHECKPOINTS[checkpointIndex + 1] || null;
        if (nextCp) {
            mapMessage.innerHTML = nextCp.hint;
        } else {
...
function handleCheckpoint() {
    ...
    // Update map after a delay
    setTimeout(() => {
        updateMap();
        updateItemsDisplay();
        setTimeout(() => updateMapMessage(), 1000);
    }, 800);
}
...
function showStickerBook() {
    ...
    if (stickers.length === 0) {
        ...
        [...stickers].reverse().forEach((s) => {
            ...
            div.innerHTML = `
        <span class="sticker-card-emoji">${s.emoji}</span>
        <span class="sticker-card-name">${s.name}</span>
        <span class="sticker-card-date">${formatDate(s.date)}</span>
      `;
...
function setupEventListeners() {
    ...
    btnDispatch.addEventListener('click', () => {
        animateButtonPress(btnDispatch);
        playButtonSound();
        showPhase(2);
    });
    ...
    // Phase 2: Go to mission
    btnGoMission.addEventListener('click', () => {
        animateButtonPress(btnGoMission);
        playButtonSound();
        showPhase(3);
    });
    ...
    // Sticker book close
    closeStickerBook.addEventListener('click', () => {
        stickerModal.classList.add('hidden');
    });
    stickerModal.querySelector('.modal-backdrop').addEventListener('click', () => {
        stickerModal.classList.add('hidden');
    });
    ...
    soundToggle.addEventListener('change', () => {
        ...
        localStorage.setItem('rescue-sound', soundToggle.checked);
    });
